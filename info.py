...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ['6032060494'].split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ['1001847058383'].split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('6032060494', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('-1001847058383')
